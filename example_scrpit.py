...
### magnification as the example, however any correlation currently avalible
### or created by the user (assuming it conforms to the template class).

### import all the modules of CHOMP. This assumes that the user has implimented
...
### Specifiy a dictionary defining the properties of a cosmology. Details on
### on each varriable can be found in defaults.py
cosmo_dict = {
    "omega_m0": 0.3,
    "omega_b0": 0.046,
    "omega_l0": 0.7,
...
### implimented and if you like the code fast I wouldn't recommend changing it.)
halo_dict = {
    "stq": 0.3,
    "st_little_a": 0.707,
    "c0": 9.,
...
### Initialize the halo object with the mass function and signle epoch cosmology
### implimentation is from Seljak2000.
halo_model = halo.Halo(redshift=0.0, input_hod=sdss_hod,
                       cosmo_single_epoch=cosmo_single)


### From this point we have fully defined our cosmology and halo model. The next
### the next step is defining the z redshift distributions and approperate 
### window functions.

### Below we define our foreground lense and background source distributions 
...
### we use a guassian with mean z=1.0. Other options could be used here, see
### kernel.py for other implimented distributions.
lens_dist = kernel.dNdzMagLim(0.0, 2.0, 2.0, 0.3, 2.0)
source_dist = kernel.dNdzGaussian(0.0, 2.0, 1.0, 0.2)

### Now we need to create the approperate window functions that will allow use 
### to project the power spectrum from halo. Currently these come in two
### variates. The first is WindowFunctionGalaxy which defines the dn/dchi
### distrubtion of galaxies. If one is intrested in the cluster of two(one)
...
### initializeing the kernel is that we need to know what value of k*theta we
...
### conviently allows for the setting of both the kernel and halo model
